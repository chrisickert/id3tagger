...
public class NormalizedFuzzyLevenshteinDistance implements Similarity {

  @Nonnull private Similarity entrySimilarity;

  private double threshold;

  public NormalizedFuzzyLevenshteinDistance(@Nonnull Similarity entrySimilarity, double threshold) {
  ...
  public double similarity(@Nonnull Term term1, @Nonnull Term term2) {
    ...
    for (j = 1; j <= length2; j++) {
      ...
      for (i = 1; i <= length1; i++) {
        ...
        double entrySimilarity = this.entrySimilarity.similarity(term1_i, term2_j);
        cost = entrySimilarity >= threshold ? 0 : 1;
        costs[i] =
